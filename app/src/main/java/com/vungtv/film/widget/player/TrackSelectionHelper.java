...
public class TrackSelectionHelper implements View.OnClickListener,
    ...
    public void showSelectionDialog(Activity activity, CharSequence title, MappedTrackInfo trackInfo,
        ...
        builder.setTitle(title)
                .setView(buildView(res, LayoutInflater.from(builder.getContext())))
    ...
    private View buildView(Resources res, LayoutInflater inflater) {
        ...
        for (int groupIndex = 0; groupIndex < trackGroups.length; groupIndex++) {
            ...
            for (int trackIndex = 0; trackIndex < group.length; trackIndex++) {
                ...
                trackView.setText(buildTrackName(res, group.getFormat(trackIndex)));
    ...
    private static String buildTrackName(Resources res, Format format) {
        String trackName;
        if (MimeTypes.isVideo(format.sampleMimeType)) {
            trackName = joinWithSeparator(buildResolutionString(res, format), buildBitrateString(format));
    ...
    private static String buildResolutionString(Resources res, Format format) {
        if (format.width == Format.NO_VALUE || format.height == Format.NO_VALUE) {
            return "";
        }

        if (format.height >= 2160) {
            return res.getString(R.string.player_text_quality_4k);
        } else if (format.height >= 1440) {
            return res.getString(R.string.player_text_quality_2k);
        } else if (format.height >= 1080) {
            return res.getString(R.string.player_text_quality_fhd);
        } else if (format.height >= 720) {
            return res.getString(R.string.player_text_quality_hd);
        } else if (format.height >= 480) {
            return res.getString(R.string.player_text_quality_480p);
        } else {
            return res.getString(R.string.player_text_quality_360p);
        }
    }
